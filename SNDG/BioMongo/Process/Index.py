...
def build_statistics(self, genome_name):
    import numpy as np
    genome = SeqCollection.objects(name=genome_name).get()  # @UndefinedVariable
    genome.statistics = []
    total = self.db.proteins.count({"organism": genome_name})

    drug = [x["search"]["druggability"] for x in self.db.proteins.find(
    ...
    for dp in genome.druggabilityParams:
        if dp.target == "pathway":
            ...
            total = self.db.proteins.count({"organism": genome_name, "search." + dp.name: {"$exists": 0}})
            if dp.type == SeqColDruggabilityParamTypes.number:
                data = [x["search"][dp.name]
                        for x in self.db.proteins.find(
                ...
                count = [self.db.proteins.count({"organism": genome_name, "search." + dp.name:
    ...
    genome.statistics.append(
        Metric(name="proteins", value=self.db.proteins.find({"seq_collection_id": genome.id}).count()))
    ecre = re.compile("^ec:")
    genome.statistics.append(
        Metric(name="ec", value=self.db.proteins.find({"seq_collection_id": genome.id, "ontologies": ecre}).count()))
    gore = re.compile("^go:")
    genome.statistics.append(
        Metric(name="go", value=self.db.proteins.find({"seq_collection_id": genome.id, "ontologies": gore}).count()))

    genome.statistics.append(Metric(name="SO:0001079", description="polypeptide_structural_motif",
                                    value=self.db.proteins.find(
    ...
    genome.statistics.append(
        Metric(name="SO:0000417", description="polypeptide_domain_count", value=self.db.proteins.find(
            {"organism": genome_name, "features.type": "SO:0000417"}).count()))
    genome.statistics.append(Metric(name="SO:0000418", description="signal_peptide_count", value=self.db.proteins.find(
        {"organism": genome_name, "features.type": "SO:0000418"}).count()))
    genome.statistics.append(
        Metric(name="SO:0100009", description="lipo_signal_peptide_count", value=self.db.proteins.find(
            {"organism": genome_name, "features.type": "SO:0100009"}).count()))
    genome.statistics.append(Metric(name="SO:0001077", description="transmembrane_count", value=self.db.proteins.find(
