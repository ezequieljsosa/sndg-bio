...
def process_strain(ref_fasta, strain, read_template, work_dir):
    """
    ...
    if not os.path.exists(out_bwa_bam_idx) and not os.path.exists(out_bwa_pe_bam):
        execute('bwa mem -R "@RG\\tID:illumina\\tSM:{ncepa}\\tLB:{ncepa}"  {ref_fasta} {pe1} {pe2}  >  ' + out_bwa_pe,
                ...
                pe1=read_template % (strain, "R1"),
                pe2=read_template % (strain, "R2"))
...
def process_ref(ref_fasta, strains, read_template, work_dir):
    ...
    for strain in strains:
        process_strain(ref_fasta, strain, read_template, work_dir + "/" + strain)


def variant_call(strain, in_bam, ref_fasta, snpeffdb, work_dir="./", cpus=1):
