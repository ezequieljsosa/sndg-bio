...
Opciones:\n\
	...
	-r	Archivo de secuencias de entrenamiento (Opcional. Autoentrenamiento por defecto).\n\
...
try:
    options, args = getopt.getopt(sys.argv[1:], "i:t:o:l:s:r:h")
...
params["r"] =  ""                    #Archivo de secuencias de entrenamiento (Opcional. Autoentrenamiento por defecto).
...
if not os.path.isfile(log):
	print "Esta iniciando el pipeline desde el Paso 2. Continuar de todos modos (s/n)? Debera ejecutar el Paso 1 antes de pasar al Paso 3."
	respuesta = raw_input()
	if respuesta == "s":
		pass
	else:
...
if params["r"]:
    print "    Usando %s como set entrenamiento" % params["r"]
else:
    print "    Usando autoentrenamiento"
...
for seq_record in SeqIO.parse(f_contigs, "fasta"):
	...
	fasta_file.close()
	if params["r"]:
		getoutput("cp  %s  tmpbia/%s.train" % (params["r"], file_prefix))
	else:
...
if params["r"]:
    params["r"] = params["r"] + "como set de entrenamiento"
else:
    params["r"] =  "autoentrenamiento"

log_step = open(log, "a")

now = time.strftime("%d/%m/%Y %H:%M:%S", time.gmtime())
print >> log_step, "[ %s ] Paso 2 ProteinPredictor corrido sobre archivo %s usando %s. Umbral de entropia para ORFs detectados por long-orfs: %s. Maximo overlap entre genes: %s. Minima longitud de gen: %s. Minimo score para ser considerado gen: %s   \n" % (now, params["i"], params["r"], params["t"], params["o"], params["l"], params["s"] )
