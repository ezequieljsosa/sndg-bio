...
def sec(product, gene, symbol_source, product_source, EC, GO):
    if getoutput("grep %s  ./sec_%s/praze.out.btab" % (ORF.id, input_name)):
        if getoutput("grep %s  ./sec_%s/praze.out.btab" % (ORF.id, input_name)) and os.path.exists(
                "sec_%s/praze.out.btab" % (input_name)) and float(
                getoutput("grep %s  ./sec_%s/praze.out.btab" % (ORF.id, input_name)).split("\n")[0].split("\t")[
                    10]) >= 40:
            bstart, bend, bhstart, bhend = \
            getoutput("grep %s  ./sec_%s/praze.out.btab" % (ORF.id, input_name)).split("\n")[0].split("\t")[6:10]
            if not paso_3:
                qstart, qend = ORF.description.strip().split()[2:4]
            else:
                qstart, qend = "1", str(len(ORF.seq) * 3)
            hstart, plus, hend, endline = \
            getoutput("grep %s  ./sec_%s/praze.out.btab" % (ORF.id, input_name)).split("\n")[0].split("\t")[16:]
            ...
            if getoutput("grep %s -m 1 %s/genesymbolmap.%s" % (uniprot, params["g"], uniprot[0:2])):
                gene, evidence = getoutput(
                    "grep %s -m 1 %s/genesymbolmap.%s" % (uniprot, params["g"], uniprot[0:2])).split()[1:]
                symbol_source = cluster + "_" + uniprot
            if int(evidence) < 5:
                if cov >= 0.8 and covh >= 0.8:
                    GOlines = getoutput("egrep \"%s\" '%/gp_association.goa_uniprot.%s'  " % (
                    uniprot, params["m"], uniprot[0:3])).split("\n")
                    GO = list(GO)
                    for line in GOlines:
                    ...
                    product = \
                    getoutput("grep %s  ./sec_%s/praze.out.btab" % (ORF.id, input_name)).split("\n")[0].strip().split(
                        "\t")[15].split("n=")[0]  # + "     UNIREF100"
                    product_source = cluster + "_" + uniprot
                else:
                    if covh < 0.8:
                        GO = "GO:0008150,GO:0003674,GO:0005575"
                    else:
                        GOlines = getoutput("egrep \"%s\" '%s/gp_association.goa_uniprot.%s'  " % (
                        uniprot, params["m"], uniprot[0:3])).split("\n")
                        GO = list(GO)
                        for line in GOlines:
                            if line and line.split("\t")[3] not in GO:
                    ...
                    product = \
                    getoutput("grep %s  ./sec_%s/praze.out.btab" % (ORF.id, input_name)).split("\n")[0].strip().split(
                        "\t")[15].split("n=")[0] + " domain protein"  # +"    UNIREF100"
                    product_source = cluster + "_" + uniprot
...
try:
    options, args = getopt.getopt(sys.argv[1:], "i:o:l:s:p:t:g:m:j:v:h")
...
if not os.path.isfile(log):
    ...
    if int(getoutput("grep \"Paso 1\" %s -c" % log)) == 0 and int(
            getoutput("grep \"Paso 2\" %s -c" % log)) == 0 and int(getoutput("grep \"Paso 3\" %s -c" % log)) >= 1:
        paso_3 = True
        pass
    else:
...
rank = {"equivalog": 1, "ber": 2, "subfamily": 3, "equivalog_domain": 4, "superfamily": 5, "subfamily_domain": 6,
        "domain": 7, "pfam": 8, "tmhmm": 9,
        "lipoprotein": 10, "hypoth_equivalog": 11, "hypoth_equivalog_domain": 1000, "signature": 1000, "repeat": 1000,
        "paralog": 1000, "paralog_domain": 1000, "exception": 1000}

SO_term = {"lipoprotein_signal_peptide": "SO:0100009", "signal_peptide": "SO:0000418", "transmembrane": "SO:0001077",
           "polypeptide_domain": "SO:0000417", "beta_strand": "SO:0001111", "peptide_helix": "SO:0001114",
           "disordered_region": "SO:0100003"}
...
for seq_record in SeqIO.parse(f_contigs, "fasta"):
    ...
    if not paso_3:
        ...
        getoutput(
            "tail -n +2 %s/curatednr_%s.blast | gawk -F '	'  '{print $1\"	fecha	some_number	praze	path_source	\"$2\"	\"$4\"	\"$5*3\"	\"$6\"	\"$7\"	\"$8\"	similarity	false_cov	X	Y	\"$3\"	1	Plus	\"$5*100/$11\"	\"}'   > %s/ber_%s/praze.out.btab" % (
            seq_record.id, input_name, seq_record.id, input_name))
        getoutput("mkdir %s/bercurated_%s " % (seq_record.id, input_name))
        getoutput(
            "tail -n +2 %s/curatednr_%s.blast | gawk -F '	'  '{print $1\"	fecha	some_number	praze	path_source	\"$2\"	\"$4\"	\"$5*3\"	\"$6\"	\"$7\"	\"$8\"	similarity	false_cov	X	Y	\"$3\"	1	Plus	\"$5*100/$11\"	\"}'   > %s/bercurated_%s/praze.out.btab" % (
            seq_record.id, input_name, seq_record.id, input_name))

    if os.path.exists("%s/sec_%s.blast" % (seq_record.id, seq_record.id)):
        getoutput("mkdir %s/sec_%s " % (seq_record.id, seq_record.id))
        getoutput(
            "tail -n +2 %s/sec_%s.blast | gawk -F '	'  '{print $1\"	fecha	some_number	praze	path_source	\"$2\"	\"$4\"	\"$5*3\"	\"$6\"	\"$7\"	\"$8\"	similarity	false_cov	X	Y	\"$3\"	1	Plus	\"$5*100/$11\"	\"}'   > %s/sec_%s/praze.out.btab" % (
            seq_record.id, seq_record.id, seq_record.id, seq_record.id))
    ...
    if not paso_3:
        while j < len(curatedhmmer_lines):
            ...
            while j < len(curatedhmmer_lines) and id == curatedhmmer_lines[j].split("\t")[0]:
                ...
                if score >= float(
                        getoutput("egrep -A 14 '\s\s%s$'  %s" % (hitid, params["p"])).split("\n")[13].strip().split()[
                            -1][0:-1]):  # Si el score supera el trusted cutoff
                    if re.match("TIGR", hitid):
                        ...
                        covh = 100 * abs(float(curatedhmmer_lines[j].split("\t")[-5].strip()) - float(
                            curatedhmmer_lines[j].split("\t")[-6].strip())) / float(
                            getoutput("egrep -A 4 '\s\s%s$'  %s" % (hitid, params["p"])).split("\n")[3].strip().split()[
                                -1])
                        # print id, covq, covh
                        ...
                        if float(score) > score_pfam:
                            ...
                            pfam_id = \
                            getoutput("egrep -A 1 '\s\s%s$'  %s" % (hitid, params["p"])).split("\n")[1].strip().split()[
                                1]
                            pfam_match[id] = [curatedhmmer_lines[j].split("\t")[2].strip(), score_pfam, beg, end,
                                              pfam_id]  # nombre, hitid, beg, end, score
                    if newisotype < isotype:
                        ...
                        tigr_class[id] = curatedhmmer_lines[j].split("\t")[
                                             3].strip(), gene_sym, EC_number, isotype, hitid  # nombre, gene_sym, EC_number, isotype, hitid
            ...
            while j < len(hmmer_lines) and id == hmmer_lines[j].split("\t")[0]:
                ...
                if score >= float(getoutput("egrep -A 14 '\s\s%s$'  '%s'  " % (hitid, params["p"])).split("\n")[
                    ...
                    if re.match("TIGR", hitid):
                        ...
                        covh = 100 * abs(float(hmmer_lines[j].split("\t")[-5].strip()) - float(
                            hmmer_lines[j].split("\t")[-6].strip())) / float(
                            getoutput("egrep -A 4 '\s\s%s$'  %s" % (hitid, params["p"])).split("\n")[3].strip().split()[
                                -1])
                        if getoutput("egrep -A 7 '\s\s%s$'  %s" % (hitid, params["p"])).split("\n")[6].strip().split()[
                            -1] == "no":
                        ...
                        if float(score) > score_pfam:
                            ...
                            if pfam_haskey == "False":
                                pfam_match[id] = [hmmer_lines[j].split("\t")[2].strip(), score_pfam, beg, end,
                                                  pfam_id]  # nombre, hitid, beg, end, score
                    if newisotype < isotype:
                        isotype = newisotype
                        if isotype != 8:
                            ...
                            if EC_number:
                                EC_number = EC_number.split()[-1]
                        tigr_class[id] = hmmer_lines[j].split("\t")[
                                             3].strip(), gene_sym, EC_number, isotype, hitid  # nombre, gene_sym, EC_number, isotype, hitid
                j += 1
        else:
            j += 1
    ...
    if not paso_3:
        gffcontig = formatogff(input_name) + "\tv0.1-bia_pap\tcontig\t1\t" + str(
            len(genome.seq)) + "\t.\t+\t.\tID=" + formatogff(
            input_name) + ";translation_table=11;organism_name=" + formatogff(gff3id) + ";abbreviation=" + formatogff(
            params["l"]) + ";defline=" + formatogff(genome.description) + ";Name=" + formatogff(input_name)
    ...
    while True:
        ...
        try:
            ...
            break
        if params["s"] == "4":
            product, gene, symbol_source, product_source, EC, GO = sec(product, gene, symbol_source, product_source, EC,
                                                                       GO)
        ...
        if params["s"] == "3":
            if (product == "" or re.search("hypothetical protein", product)):
                product, gene, symbol_source, product_source, EC, GO = sec(product, gene, symbol_source, product_source,
                                                                           EC, GO)

        if product == "" and getoutput("grep %s  %s/ber_%s/praze.out.btab" % (ORF.id, seq_record.id, seq_record.id)):
            if getoutput("grep %s  %s/bercurated_%s/praze.out.btab" % (
            ORF.id, seq_record.id, seq_record.id)) and os.path.exists(
                    "%s/bercurated_%s/praze.out.btab" % (seq_record.id, seq_record.id)) and float(getoutput(
                    "grep %s  %s/bercurated_%s/praze.out.btab" % (ORF.id, seq_record.id, seq_record.id)).split("\n")[
                                                                                                      0].split("\t")[
                                                                                                      10]) >= 40:
                bstart, bend, bhstart, bhend = \
                getoutput("grep %s  %s/bercurated_%s/praze.out.btab" % (ORF.id, seq_record.id, seq_record.id)).split(
                    "\n")[0].split("\t")[6:10]
                if not paso_3:
                    qstart, qend = ORF.description.strip().split()[2:4]
                else:
                ...
                getoutput("grep %s  %s/bercurated_%s/praze.out.btab" % (ORF.id, seq_record.id, seq_record.id)).split(
                    "\n")[0].split("\t")[16:]
                cov = (float(bend) - float(bstart) + 1) / (
                        abs(float(qend) - float(qstart)) + 1)  # cobertura en el query
                covh = (float(bhend) - float(bhstart) + 1) / (
                        abs(float(hend) - float(hstart)) + 1)  # cobertura en el hit
                cluster, uniprot = \
                getoutput("grep %s  %s/bercurated_%s/praze.out.btab" % (ORF.id, seq_record.id, seq_record.id)).split(
                    "\n")[0].split("\t")[5].split("_")
                evidence = 5
                ...
                if int(evidence) < 5:
                    ...
                    "grep %s  %s/ber_%s/praze.out.btab" % (ORF.id, seq_record.id, seq_record.id)) and os.path.exists(
                    "%s/ber_%s/praze.out.btab" % (seq_record.id, seq_record.id)) and float(
                    getoutput("grep %s  %s/ber_%s/praze.out.btab" % (ORF.id, seq_record.id, seq_record.id)).split("\n")[
                        0].split("\t")[10]) >= 40:
                if float(getoutput("grep %s  %s/ber_%s/praze.out.btab" % (ORF.id, seq_record.id, seq_record.id)).split(
                    ...
                    if int(evidence) < 5:
                        if cov >= 0.8 and covh >= 0.8:
                            ...
                            GO = ",".join(GO)
                            product = getoutput(
                                "grep %s  %s/ber_%s/praze.out.btab" % (ORF.id, seq_record.id, seq_record.id)).split(
                                "\n")[0].strip().split("\t")[15].split("n=")[0]  # +  "     UNIREF100"
                            product_source = cluster + "_" + uniprot
                        else:
                            if covh < 0.8:
        ...
        if product == "" or (tigr_class.has_key(ORF.id) and tigr_class[ORF.id][3] == 11):
            if tmhmmpred and float(tmhmmpred.split("\t")[2].split("=")[1]) / float(
                    tmhmmpred.split("\t")[1].split("=")[1]) >= 0.5:
        ...
        if product == "" or (tigr_class.has_key(ORF.id) and tigr_class[ORF.id][3] == 11):
            ...
            if lipopred:
                ...
                lipomargin = float(
                    getoutput("grep %s  %s/orfs_%s.lipop" % (ORF.id, seq_record.id, seq_record.id)).split("\n")[
        ...
        if gene == "":
            if getoutput(
                    "grep %s  %s/ber_%s/praze.out.btab" % (ORF.id, seq_record.id, seq_record.id)) and os.path.exists(
                    "%s/ber_%s/praze.out.btab" % (seq_record.id, seq_record.id)):
                cluster, uniprotid = \
                getoutput("grep %s  %s/ber_%s/praze.out.btab" % (ORF.id, seq_record.id, seq_record.id)).split("\n")[
                    0].split("\t")[5].split("_")
                if getoutput("grep %s -m 1 %s/genesymbolmap.%s" % (uniprotid, params["g"], uniprotid[0:2])):
                    gene = \
                    getoutput("grep %s -m 1 %s/genesymbolmap.%s" % (uniprotid, params["g"], uniprotid[0:2])).split()[1]
        ...
        gffgene = formatogff(
            input_name) + "\tv0.1bia_pap\tgene\t" + beg + "\t" + end + "\t.\t" + dir + "\t.\tID=" + formatogff(
            gff3id) + ".gene." + input_number + str(i) + db_ref
        gffexon = formatogff(
            input_name) + "\tv0.1bia_pap\texon\t" + beg + "\t" + end + "\t.\t" + dir + "\t0\tID=" + formatogff(
            gff3id) + ".exon." + input_number + str(i) + ";Parent=" + formatogff(
            gff3id) + ".transcript." + input_number + str(i)
        gffmRNA = formatogff(
            input_name) + "\tv0.1bia_pap\tmRNA\t" + beg + "\t" + end + "\t.\t" + dir + "\t.\tID=" + formatogff(
            gff3id) + ".transcript." + input_number + str(i) + ";Parent=" + formatogff(
            gff3id) + ".gene." + input_number + str(i)
        if symbol_source:
        ...
        gffmRNA = gffmRNA + ";Note=" + product + ";description=" + product + ";Name=" + formatogff(
            gff3id) + ".transcript." + input_number + str(i) + ";locus_tag=" + params["l"] + "_" + str(i)
        gffCDS = formatogff(
            input_name) + "\tv0.1bia_pap\tCDS\t" + beg + "\t" + end + "\t.\t" + dir + "\t0\tID=" + formatogff(
            gff3id) + ".CDS." + input_number + str(i) + ";Parent=" + formatogff(
            gff3id) + ".transcript." + input_number + str(i)
        if dict_blast_lines_cog.has_key(ORF.id) and dict_blast_lines_cog[ORF.id]:
        ...
        if pfam_match.has_key(ORF.id):
            ...
            gffpfam = formatogff(input_name) + "\tv0.1bia_pap\tpolypeptide_domain\t" + str(dom_beg) + "\t" + str(
                dom_end) + "\t" + str(pfam_match[ORF.id][1]) + "\t" + dir + "\t." + "\tID=" + formatogff(
                gff3id) + ".domain." + input_number + str(i) + ";Parent=" + formatogff(
                gff3id) + ".gene." + input_number + str(i) + ";Ontology_term=" + formatogff(
                SO_term["polypeptide_domain"]) + ";description=" + str(pfam_match[ORF.id][0]) + ";Accession=" + str(
                pfam_match[ORF.id][4])
        ...
        if tmhmmpred:
            ...
            for segment in top_array:
                ...
                gfftmhmm = formatogff(input_name) + "\tv0.1bia_pap\ttransmembrane\t" + str(seg_beg) + "\t" + str(
                    seg_end) + "\t.\t" + dir + "\t." + "\tID=" + formatogff(
                    gff3id) + ".transmembrane." + input_number + str(i) + "." + str(h) + ";Parent=" + formatogff(
                    gff3id) + ".gene." + input_number + str(i) + ";Ontology_term=" + formatogff(
                    SO_term["transmembrane"])
        ...
        if lipopred:
            ...
            if lipopredT == "SpII" and lipomargin >= 3:
                ...
                gfflipop = formatogff(input_name) + "\tv0.1bia_pap\tlipoprotein_signal_peptide\t" + str(
                    cleavbeg) + "\t" + str(cleavend) + "\t.\t" + dir + "\t." + "\tID=" + formatogff(
                    gff3id) + ".lipoprotein." + input_number + str(i) + ";Parent=" + formatogff(
                    gff3id) + ".gene." + input_number + str(i) + ";Ontology_term=" + formatogff(
                    SO_term["lipoprotein_signal_peptide"])
        ...
        if signalpred:
            ...
            gffsignalp = formatogff(input_name) + "\tv0.1bia_pap\tsignal_peptide\t" + str(cleavbeg) + "\t" + str(
                cleavend) + "\t.\t" + dir + "\t." + "\tID=" + formatogff(
                gff3id) + ".signalpeptide." + input_number + str(i) + ";Parent=" + formatogff(
                gff3id) + ".gene." + input_number + str(i) + ";Ontology_term=" + formatogff(SO_term["signal_peptide"])
        ...
        if ORF.id in psipred:
            ...
            for region in psipred[ORF.id]:
                ...
                gffpsipred = formatogff(input_name) + "\tv0.1bia_pap\t" + structure[stype] + "\t" + str(
                    seg_beg) + "\t" + str(seg_end) + "\t.\t" + dir + "\t." + "\tID=" + formatogff(gff3id) + "." + \
                             structure[stype] + "." + input_number + str(i) + "." + str(h) + ";Parent=" + formatogff(
                    gff3id) + ".gene." + input_number + str(i) + ";Ontology_term=" + formatogff(
                    SO_term[structure[stype]])
        ...
        if ORF.id in disopred:
            ...
            for region in disopred[ORF.id]:
                ...
                gffdisopred = formatogff(input_name) + "\tv0.1bia_pap\t" + structure[stype] + "\t" + str(
                    seg_beg) + "\t" + str(seg_end) + "\t.\t" + dir + "\t." + "\tID=" + formatogff(gff3id) + "." + \
                              structure[stype] + "." + input_number + str(i) + "." + str(h) + ";Parent=" + formatogff(
                    gff3id) + ".gene." + input_number + str(i) + ";Ontology_term=" + formatogff(
                    SO_term[structure[stype]])
        ...
        if not paso_3:
            ...
            ORF.id = "bia|" + ORF.id + "_" + genome.id + " " + product + " OS=" + params[
                "o"] + " GN=" + gene + " GO=" + GO
    ...
    if not paso_3:
        ...
        while True:
            ...
            gffexon = formatogff(
                input_name) + "\tv0.1bia_pap\texon\t" + beg + "\t" + end + "\t.\t" + dir + "\t0\tID=" + formatogff(
                gff3id) + ".exon." + input_number + str(i)
            gfftRNA = formatogff(
                input_name) + "\tv0.1bia_pap\ttRNA\t" + beg + "\t" + end + "\t.\t" + dir + "\t.\tID=" + formatogff(
                gff3id) + ".tRNA." + input_number + str(
                i) + ";tRNA_anti-codon=" + anticodon + ";score=" + tRNAscore + ";Note=" + tRNA_product + ";description=" + tRNA_product + ";locus_tag=" + \
                      params["l"] + "_" + str(i)
            gffCDS = formatogff(
                input_name) + "\tv0.1bia_pap\tCDS\t" + beg + "\t" + end + "\t.\t" + dir + "\t0\tID=" + formatogff(
                gff3id) + ".CDS." + input_number + str(i)
            print >> GFF3, gffgene
            print >> GFF3, gffexon
            print >> GFF3, gfftRNA
            print >> GFF3, gffCDS
        ...
        while True:
            ...
            gffexon = formatogff(
                ...
                gff3id) + ".exon." + input_number + str(i)
            gfftRNA = formatogff(
                input_name) + "\tv0.1bia_pap\trRNA\t" + beg + "\t" + end + "\t.\t" + dir + "\t.\tID=" + formatogff(
                gff3id) + ".rRNA." + input_number + str(
                i) + ";score=" + rRNAscore + ";Note=" + rRNA_product + ";description=" + rRNA_product + ";locus_tag=" + \
                      params["l"] + "_" + str(i)
            gffCDS = formatogff(
                input_name) + "\tv0.1bia_pap\tCDS\t" + beg + "\t" + end + "\t.\t" + dir + "\t0\tID=" + formatogff(
                gff3id) + ".CDS." + input_number + str(i)
    ...
    if not paso_3:
        print (getoutput(
            "tbl2asn -j \"[gcode=11]\" -k m  -Z discrepancies  -t ncbi.sbt -V vb -i %s/ncbi_%s.fna  -f  %s/ncbi_%s.tbl  -k m" % (
            seq_record.id, seq_record.id, seq_record.id, seq_record.id)))

    # Generacion de archivo .gff3
    print getoutput(
        "cat %s/ncbi_%s.gff3.features > %s/ncbi_%s.gff3" % (seq_record.id, seq_record.id, seq_record.id, seq_record.id))
    if not paso_3:
        print (getoutput("cat %s/ncbi_%s.gff3.fasta >> %s/ncbi_%s.gff3" % (
        seq_record.id, seq_record.id, seq_record.id, seq_record.id)))
    getoutput("rm  %s/ncbi_%s.gff3.features && rm %s/ncbi_%s.gff3.fasta" % (
    seq_record.id, seq_record.id, seq_record.id, seq_record.id))

now = time.strftime("%d/%m/%Y %H:%M:%S", time.gmtime())
print >> log_step, "[ %s ] Paso 8 Anotator corrido sobre archivo %s. Organismo: %s. Locus tag para los genes: %s" % (
now, params["i"], params["o"], params["l"])
...
getoutput("cat  */*fna  > ncbi_%s.fna" %  prefix)
getoutput("cat  */*faa  > ncbi_%s.faa" %  prefix)
getoutput("cat  */*sqn  > ncbi_%s.sqn" %  prefix)
