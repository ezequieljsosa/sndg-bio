...
from aux_libs import BackgroundBlast
...
try:
    options, args = getopt.getopt(sys.argv[1:], "i:d:e:a:h")
...
if not os.path.isfile(log):
    print "Advertencia. Esta iniciando el pipeline desde el Paso 4."
    warning = True

else:
    if int(getoutput("grep \"Paso 1\" %s -c" % log)) == 0 and int(
            getoutput("grep \"Paso 2\" %s -c" % log)) == 0 and int(getoutput("grep \"Paso 3\" %s -c" % log)) >= 1:
        print "Advertencia: El pipeline se ha comenzado desde el paso 3 (usando set de proteinas preexistente). Debe continuar con el paso 5.\n";
        warning = True
        ...
        if int(getoutput("grep \"Paso 3\" %s -c" % log)) == 0:
            ...
            warning = True
        if int(getoutput("grep \"Paso 4\" %s -c" % log)) >= 1:
            print "Advertencia: ya se ejecuto el Paso 4.\n"
            warning = True
if warning == True:
...
for seq_record in SeqIO.parse(f_contigs, "fasta"):
    ...
    blastout = 'tmpbia/nr_%s.blast' % seq_record.id

    if os.path.exists('tmpbia/nr_%s.blast' % seq_record.id):
        print '\tUsando archivo tmpbia/nr_%s.blast previamente calculado' % seq_record.id
    else:
        Bblast = BackgroundBlast("%s/orfs_%s.faa" % (seq_record.id, seq_record.id), params["d"], blastout, params["a"])
        Bblast.run()

        # getoutput("blastall -p blastp -i '%s/orfs_%s.faa' -d %s -e %s -a %s -v 20 -b 20 -o '%s' " % (seq_record.id, seq_record.id,params["d"],params["e"],params["a"],blastout))
        ...
        while progreso < 100:
            if os.path.exists(blastout):
                blast_count = int(getoutput("grep 'Query=' -c %s" % blastout))

            lapso = tiempo_transcurrido(begin)
            if orfs_count != 0:
                progreso = float(100 * blast_count / orfs_count)
            else:
                progreso = 100
            if lapso[0] == "0":
                print "\t\tAlineamiento %.2f%s en %sd %sh %sm %ss \r" % (
                progreso, "%", lapso[0], lapso[1], lapso[2], lapso[3]),
            else:
                print "\tAlineamiento %.2f%s en %sh %sm %ss \r" % (progreso, "%", lapso[1], lapso[2], lapso[3]),
    ...
    try:
        for blast_record in blast_iterator:
            ...
            i += 1
            progreso = float(100 * i / orfs_count)
            print "\t\tFiltrado %.2f%s  \r" % (progreso, "%"),
            for alignment in blast_record.alignments:  # Para cada alineamiento de blastout
                ...
                for hsp in alignment.hsps:  # Para cada hsp en cada alineamiento
                    cov = int(100 * (float(hsp.query_end) - float(hsp.query_start) + 1.0) / len(
                        queryrec.seq))  # Calculo la cobertura
                    ...
                    hits.append((queryrec.id, queryrec.description, sbjctID, sbjttProd, dfrom, dto, tfrom, tto, ident,
                                 cov, evalue, score))
    except:
        pass
    ...
    if len(hits) != 0:
        ...
        getoutput(
            "ber -i 'tmpbia/nr_%s.blast' -d %s -f formatdb -D  '%s/orfs_%s.flanking.fna'  -F formatdb  -m  '%s/orfs_%s.map'   -e 1e-5 -E 1e-5  -n 20  -N 0" % (
            seq_record.id, params["d"], seq_record.id, seq_record.id, seq_record.id, seq_record.id))
                ...
                for starts in startsites[orf][:]:
                    ...
                    for pos in range(0, 10):
                        if float(len(re.findall("A|a|G|g", rbsrange[pos:pos + 6]))) / 6 >= 0.75:
                            ...
                            if seqstart == "atg":
                                starts[1] += 1 + float(startuseatg)
                            elif seqstart == "gtg":
                                starts[1] += 1 + float(startusegtg)
                            elif seqstart == "ttg":
                                starts[1] += 1 + float(startusettg)
                            break
                startsites[orf].sort(key=lambda start: start[1], reverse=True)
                for starts in startsites[orf][:]:
    ...
    trnafile.close()
    orffile.close()
    neworffile = open("tmpbia/orfs_%s.faa" % (seq_record.id), "w")
    RNAs = []
    for rRNA in rRNAs:
    ...
    ORFsov = []
    for ORF in ORFs:
        orfid, contigID, beg, end, score = ORF.description.strip().split()[:5]
        if int(beg) > int(end):
            beg, end = end, beg
    ...
    for ORF in ORFs:
        ...
        if startsites.has_key(orfid):
            if beg < end:
                beg_new = beg + startsites[orfid][0][0] - (flank + 1)
            else:
                beg_new = beg - startsites[orfid][0][0] + (flank + 1)
        ...
        if orfid in blasthits[:] or orfid in hmmhits[:] or not overlap:
            neworf = SeqRecord(Seq(protseq), id=orfid,
                               description="%s %d %d %s %s" % (contigID, beg, end, score, curated))
            SeqIO.write(neworf, neworffile, "fasta")
    neworffile.close()

    getoutput("mv out %s/ber_%s" % (seq_record.id, seq_record.id))
    getoutput("cp  tmpbia/orfs_%s.faa  %s/" % (seq_record.id, seq_record.id))
    ...
    while j < len(interev):
        ...
        orf = SeqRecord(Seq(interev[j]), id="orf%.5d" % (seqname + 1 + j),
                        description="%s %d %d" % (contigID, beg, end))
    ...
    queryfile.close()
    ## Ejecucion de blastx
    # getoutput("blastall -p blastx -i '%s/interevidence_%s.fna' -d %s -e %s -a 4 -v 20 -b 20 -o tmpbia/interevidence_%s.blast" % (seq_record.id, seq_record.id, params["d"],params["e"], seq_record.id))
...
print >> log_step, "[ %s ] Paso 4 BlastHits corrido sobre archivo %s usando %s como base de datos de proteinas. Umbral de entropia e-value: %s.\n" % (
now, params["i"], params["d"], params["e"])
log_step.close()
