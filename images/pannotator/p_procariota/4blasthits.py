...
Opciones:\n\
	...
	-d 	Base de datos para busqueda de similitud. Default: data/uniprot/uniref100/uniref100.fasta\n\
...
params["d"] = "/data/uniprot/uniref100/uniref100.fasta" #Base de datos por defecto
...
for seq_record in SeqIO.parse(f_contigs, "fasta"):
	...
	if os.path.exists( 'tmpbia/nr_%s.blast' % seq_record.id):
		...
		while progreso < 100:
			...
			if orfs_count != 0:
				...
				progreso = 100
			if lapso[0] == "0":
				print "\t\tAlineamiento %.2f%s en %sd %sh %sm %ss \r"  % (progreso, "%", lapso[0], lapso[1], lapso[2], lapso[3]),
			else:
	...
	if len(hits)!=0:
		...
		getoutput("ber -i 'tmpbia/nr_%s.blast' -d %s -f formatdb -D  '%s/orfs_%s.flanking.fna'  -F formatdb  -m  '%s/orfs_%s.map'   -e 1e-5 -E 1e-5  -n 20  -N 0"    %   (seq_record.id, params["d"], seq_record.id, seq_record.id,seq_record.id, seq_record.id))
