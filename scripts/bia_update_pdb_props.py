...
class LoggerProteinListener():
    def __init__(self):
        self.count = 0
        
    def __call__(self,protein):
        self.count = self.count + 1
        _log.debug( "%s -> %i " % (protein.name,self.count) )
    ...
    for cluster_name, seqs in CDHit().clustered_seq_iterator("/data/databases/pdb/processed/seqs_from_pdb95.fasta"):
        ...
        for seq in seqs: 
            ...
            cluster.parts.append(BioProperties(pdb=pdb, chain=chain, start=start, end=end,
                                               seq_start=seq_start,seq_end=seq_end,clust_start=clust_start ,clust_end=clust_end))              
...
#             if not strdoc.residue_set("drug_binding"):
            ...
            if len(df_drug_dist_pdb_near) :
                ...
                residue_list = list(set([row.chain + "_" +  str(row.prot_res) for i , row in df_drug_dist_pdb_near.iterrows() ]))  #  @UnusedVariable
...
def update_binding_residues():
    str_num = 0
    df_binding_dist = pd.read_table("/data/pdb/distances2.tbl",sep="\t")
    ...
    for pdb,df_binding_dist_pdb in groups:
        ...
        try:            
            for r_comp_type in ['LIPID', 'METAL', 'NUCLEOTIDE', 'SUGAR',"DRUG","COFACTOR"]:
                ...
                df_comp_dist_pdb_near = df_binding_dist_pdb[(df_binding_dist_pdb.distance <= 3)  &  (comp_type == df_binding_dist_pdb.comptype ) ]
                if len(df_comp_dist_pdb_near) :
                    strdoc = ExperimentalStructure.objects(name=pdb).get()        
                    residue_list = list(set([row.chain + "_" +  str(row.prot_res) for i , row in df_comp_dist_pdb_near.iterrows() ]))  #  @UnusedVariable
        ...
        if not (strdoc.residue_set("free_cys") or strdoc.residue_set("free_tyr")):        
            ...
            for x in bp_pdb.get_residues():
                if x.resname in codes:
                    ...
                    if (codes[x.resname] in x) and (not any(map(lambda atom: (x[codes[x.resname]] - atom) <= 3, neighbor_atoms))):
...
def update_csa():
    ...
    for strdoc in ExperimentalStructure.objects():
        ...
        if len(pdb_csa) > 0:                        
                csas = [ (row["CHAIN ID"]) + "_" + str(row["RESIDUE NUMBER"])  for i,row in pdb_csa.iterrows()]  # @UnusedVariable
...
#         update_csa()
# 
#         # domains / important pfam residues
#         important_pfam()
#         logger.info("finished processing important_pfam")
# #         # cyc/tyr, all/free
#         free_cys_tyr()
#         logger.info("finished processing free_cys_tyr")
...
if __name__ == "__main__":
    ...
    if PROFILE:
        import cProfile
        import pstats
        profile_filename = 'scripts.load_bia_proteome_profile.txt'
