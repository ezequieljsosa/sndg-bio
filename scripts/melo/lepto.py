...
basepath = "/data/projects/leptostructs/"
...
with open(basepath + "/clust_dict.json") as h:
    clusters1 = json.load(h)
clusters = {}
for k,vs in clusters1.items():
    for v in vs:
        clusters[v] = k



for organism in ["LeptoBov", "Lepto-CLM-U50", "Lepto-CLM-R50"]:
    print organism
    with tqdm(model_files) as pbar:
        for model_file in pbar:
            ...
            org_model_name = model_name.replace(seq_name, clusters.get(seq_name,seq_name))
            template_name = model_name.split(seq_name + "_")[1]
            ...
            aln = [hit[0] for hit in list(bpsio.read(basepath + "/" + clusters.get(seq_name,seq_name) + "/profile_search.xml", "blast-xml")) if
